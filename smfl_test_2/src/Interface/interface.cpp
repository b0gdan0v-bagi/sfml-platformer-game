...
void GameInterface::create(RenderWindow& window, Font& FONT)
{
	...
	m_defeatText.setString("WASTED");
	m_defeatText.setFont(m_font);
	m_defeatText.setFillColor(Color::Red);
	m_defeatText.setCharacterSize(m_fontSize * 2);
...
void GameInterface::composeButtons(std::vector<Text*> BUTTON)
{
	...
	m_defeatText.setPosition(m_Shape.getPosition().x + m_Shape.getGlobalBounds().width / 2 - m_defeatText.getGlobalBounds().width / 2,
		m_Shape.getPosition().y - m_fontSize * 2 - m_defeatText.getGlobalBounds().height);
...
void GameInterface::draw(RenderWindow& window, bool& EXIT)
{
	if (m_active) 
	{
		int lastButtonRemover = 0;
		update(window);
		window.setMouseCursorVisible(true);
		window.draw(m_Shape);
		if (m_defeatTextVisible) {
			window.draw(m_defeatText);
			lastButtonRemover = 1;
		}
		m_Shape.setSize(Vector2f(m_buttons[0]->getGlobalBounds().width + 10, 15 + (3-lastButtonRemover) * m_buttons[0]->getGlobalBounds().height));
		for (int i = 0; i < m_buttons.size() - lastButtonRemover; ++i) 
		{ 
